...
create table boleto(
idBoleto int auto_increment primary key not null,
valor decimal(10,2),
vencimento date,
posicao boolean,
idMorador int,
...
select avisos.idAviso, avisos.descricaoAviso, predio.nomePredio  from avisos inner join predio on predio.idPredio = avisos.idPredio;
...
select predio.idPredio from morador inner join apto on morador.idApto = apto.idApto inner join bloco on apto.idBloco = bloco.idBloco inner join predio on bloco.idPredio = predio.idPredio where nomePredio  = "FMU";

select nomePredio from predio inpredioner join bloco on bloco.idPredio = predio.idPredio inner join apto on apto.idBloco = bloco.idBloco inner join morador on apto.idApto = morador.idApto where idMorador = 2;
